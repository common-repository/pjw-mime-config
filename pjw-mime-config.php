...
/*  Copyright 2006-11  Peter Westwood  (email : peter.westwood@ftwr.co.uk)
...
class pjw_mime_config
{
	...
	function pjw_mime_config() {
		...
		add_option('pjw_mime_types', $this->default_types, '', 'no');
		...
		else
		{
				...
				?>
					...
					<table id="mime-types" width="100%" cellpadding="3" cellspacing="3">
						...
						<?php
							...
							{
								$url = wp_nonce_url( menu_page_url( 'pjw-mime-config') . '&amp;action=delmime&amp;mime-ext=' . $ext , 'delete-mime_' . $ext );
								?>
									<tr class='alternate'>
			...
			else
			{
				?>
					...
					<form name="restoremime" id="restoremime" action="<?php menu_page_url( 'pjw-mime-config'); ?>" method="post">
			...
			?>
					...
					<form name="addmime" id="addmime" action="<?php menu_page_url( 'pjw-mime-config'); ?>" method="post">
	...
	video/x-flv flv
					...
					<form enctype="multipart/form-data" action="<?php menu_page_url( 'pjw-mime-config'); ?>" method="post">
